# Docker Control Center v1.1.1

## EN
This production update expands remote/embedded support and the container catalog.

### Highlights
- Theme + neon palette controls in Configure -> Application (static or animated).
- Improved Balena OS autostart detection (managed services show correctly).
- Added Info menu with app version, changelog, and update checks.
- SSH authentication: key + passphrase (additional key protection).
- Balena OS auto-detection (balena ps / balena run).
- Expanded catalog to 40+ popular templates, with Raspberry Pi-friendly and heavier x86_64 options.

### SSH key + passphrase (quick steps)
